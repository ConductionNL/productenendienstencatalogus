# productenendienstencatalogus chart

The Helm chart installs productenendienstencatalogus and by default the following dependencies using subcharts:
...
```bash
helm repo add productenendienstencatalogus https://raw.githubusercontent.com/ConductionNL/productenendienstencatalogus/master/api/helm/
helm repo update
```
...
```bash
helm install my-productenendienstencatalogus ConductionNL/productenendienstencatalogus --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `productenendienstencatalogus` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `productenendienstencatalogus` |
